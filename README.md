...
Bonus features that we have decided to include (but are not crucial to the core functionality of Tetris) are a scorekeeping display (which includes point bonus for accelerating the descent of the shapes), the ability for a user to choose the level they want to play, display of the next shape to be played, the availability of help and pause screens, and background music.
